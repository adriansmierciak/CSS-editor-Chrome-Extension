        ...
        chrome.storage.sync.get(['theme'], function(result) {
            ...
            chrome.storage.sync.get(['size'], function(result) {
                ...
                function allToOpenEditor() {
                    ...
                    // klikniecie na wybrany element
                    $allDOMElements.on('click', function (e) {
                        e.preventDefault();
                        e.stopPropagation();

                        //wylacza mozliwosc klikania na element(na stronie) i otwieranie okna edycji
                        $allDOMElements.off('click');
                        $allDOMElements.off('mouseenter');
                        $allDOMElements.off('mouseleave');
                        $allDOMElements.removeClass('chrome-1204-outline-selected');
                        ...
                        let $clickedElement = $(this), $editWindow = $('.chrome-1204-edit-window');
                ...
                if (!$('.chrome-1204-edit-window').length) {
                    ...
                    // outliny na zaznaczonym elemencie
                    $allDOMElements.on('mouseenter', function () {
                        $(this).addClass('chrome-1204-outline-selected');
                        $(this).parents().addClass('chrome-1204-no-outline');
                    });

                    $allDOMElements.on('mouseleave', function () {
                        $(this).removeClass('chrome-1204-outline-selected chrome-1204-no-outline');
                    });
